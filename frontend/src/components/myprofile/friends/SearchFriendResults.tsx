                ...
                <div className="h-48 md:h-56 max-w-sm bg-white border border-gray-200 rounded-lg shadow p-1 md:p-2 " key={person.friendId}>
                    <div className=" h-full flex flex-col justify-between items-center p-2">
                        ...
                        <div className='flex flex-col justify-start items-center'>
                            <h6 className="text-sm md:text-base font-medium text-gray-900 text-center">{person.firstname + " " + person.lastname}</h6>
                            <span className="text-xs text-gray-500">@{person.username}</span>
                        </div>
                        <div className="w-full flex mb-0 justify-center items-end">
                            {!friends.friendsUsernames.includes(person.username) &&
                                < button
                                    type='button'
                                    onClick={() => addFriendHandler(person.username)}
                            ...
                            }
                            {
                                friends.friendsUsernames.includes(person.username) && (
                                    <div className="w-3/5 flex justify-between">
                                        <button onClick={()=>deleteFriendHandler(person.username)} type='button' aria-label='Delete Friend'><FaCheckCircle color='green' size={25} /></button>
